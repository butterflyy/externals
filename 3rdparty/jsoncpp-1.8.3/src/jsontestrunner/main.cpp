...
int main(int argc, const char* argv[]) {
	...
	root["is_add_image_flags"] = true;
	root["is_stop_sleep_mode"] = false;
	root["image_sampling_rate"] = 15;
	root["video_sampling_rate"] = 62.3;
	root["sleep_duration_time"] = 1;

	std::string json_file = Json::writeString(writer, root);
	...
	for (std::map<std::string, int>::const_iterator it = gg.begin();
		it != gg.end();
		++it){
